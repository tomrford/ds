...
`crates/kernel/tests/v2_golden.txt` contains 32 frozen objects and IDs emitted
by the v2 codec, generated by importing a real repository (mint, ~90 commits)
through v2's git import and walking the stored history. Vectors the import
cannot produce are synthesized through the same codec: signed commits (v2's
git import drops signatures), conflicted root trees with labels, merge commits
with predecessors, and executable and submodule tree entries. The dumper is
`tests/golden_dump.rs` in the v2 repository; regenerate with
`GOLDEN_OUT=<fixture> cargo test --test golden_dump -- --ignored` there.
