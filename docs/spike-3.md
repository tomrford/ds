# Spike 3: projection proof

Phase 3 proves the 2 Git projection invariants in the v3 plan against the
machine-store path and the repository Durable Object:
...
nix develop -c pnpm exec wrangler dev --port 8787 --var SPIKE_TOKEN:dev-token
DEVSPACE_SPIKE_URL=http://127.0.0.1:8787 \
DEVSPACE_SPIKE_TOKEN=dev-token \
...
fixed activation high-water. These are spike safety limits, not settled
production quotas.

Phase 3 adds no code to the warm repository-open path. The inherited
release-only comparison remains inside the 2 times budget; 3 current local
runs measure 1.300, 1.297 and 1.297 times stock jj. The complete command
benchmark remains open because this repository still has no v3 command runner. The required
future gate is a representative warm command at no more than 2 times local jj
with the network disabled and zero cloud requests.
...
- Authentication is still one shared spike bearer token. The fencing protocol
  proves stale-owner mechanics, not cryptographic machine identity.
- The live proof omits the first callback at the protocol boundary; it does not
  launch and hard-kill a separate CLI process because the v3 command runner
  does not exist.
- Fetch-side hidden-lineage lifting, remote replacement and production Git
  credential handling remain product implementation, not projection-spike
  invariants.
