import kernelModule from "../dist/kernel-git.wasm";
...
interface KernelGitExports extends WebAssembly.Exports {
...
export interface KernelGitResult {
  id: Uint8Array;
  references: Array<{ kind: number; id: Uint8Array }>;
}

export class KernelGit {
  private exports: KernelGitExports;
  ...
  validate(kind: number, bytes: Uint8Array): KernelGitResult {
    return this.call(bytes, (pointer, length) =>
      this.exports.kernel_validate(kind, pointer, length),
    );
  }

  validateView(bytes: Uint8Array): KernelGitResult {
  ...
  validateOperation(bytes: Uint8Array): KernelGitResult {
    return decodeOpKernelResult(
      this.callRaw(bytes, (pointer, length) =>
  ...
  startHash(): KernelGitHash {
    return new KernelGitHash(this.exports);
  }
...
export class KernelGitHash {
  private state: number | undefined;

  constructor(private readonly exports: KernelGitExports) {
...
function instantiate(): KernelGitExports {
  return new WebAssembly.Instance(kernelModule, {}).exports as KernelGitExports;
}

function decodeKernelResult(bytes: Uint8Array): KernelGitResult {
...
function decodeOpKernelResult(bytes: Uint8Array): KernelGitResult {
  if (bytes[0] === 1) throw new Error(new TextDecoder().decode(bytes.subarray(1)));
